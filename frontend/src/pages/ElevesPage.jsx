...
export default function ElevesPage() {
  ...
  useEffect(() => { 
    import("../api/entities").then(({ Eleve, Classe }) => {
      Promise.all([Eleve.list(), Classe.list()]).then(([e, c]) => {
        setEleves(e);
        setClasses(c);
      }).catch(() => {});
    });
  ...
  async function save() {
    import("../api/entities").then(async ({ Eleve }) => {
  ...
  async function del(id) { 
    if (window.confirm("Supprimer cet élève ?")) { 
      import("../api/entities").then(async ({ Eleve }) => {
        await Eleve.delete(id); setEleves(eleves.filter(e => e.id !== id)); 
      });
    } 
  }
  function edit(e) { 
    setForm({ nom: e.nom||"", prenom: e.prenom||"", date_naissance: e.date_naissance||"", sexe: e.sexe||"M", adresse: e.adresse||"", telephone: e.telephone||"", email: e.email||"", nom_parent: e.nom_parent||"", tel_parent: e.tel_parent||"", numero_matricule: e.numero_matricule||"", statut: e.statut||"Actif", classe_id: e.classe_id||"" }); 
    setEditId(e.id); setShowForm(true); 
  }

  const filtered = eleves.filter(e => `${e.nom} ${e.prenom} ${e.numero_matricule}`.toLowerCase().includes(search.toLowerCase()));
  const getClasse = id => { const c = classes.find(c => c.id === id); return c ? c.nom : "—"; };

  return (
    <div className="fade-in">
      <div className="page-header">
        <h1 className="page-title"><span className="page-title-icon">👨‍🎓</span> Gestion des Élèves</h1>
        <button className="btn btn-primary" onClick={() => { setShowForm(true); setEditId(null); setForm({...emptyForm, classe_id: ""}); }}>+ Nouvel élève</button>
      </div>
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowForm(false)}>
          <div className="modal-box">
            ...
            <div className="form-grid">
              ...
              <div><label className="form-label">Sexe</label><select className="form-input" value={form.sexe||"M"} onChange={e => set("sexe", e.target.value)}><option value="M">Masculin</option><option value="F">Féminin</option></select></div>
              <div><label className="form-label">Statut</label><select className="form-input" value={form.statut||"Actif"} onChange={e => set("statut", e.target.value)}>{["Actif","Inactif","Transféré","Diplômé"].map(s => <option key={s}>{s}</option>)}</select></div>
              <div>
                <label className="form-label">Classe</label>
                <select className="form-input" value={form.classe_id||""} onChange={e => set("classe_id", e.target.value)}>
      ...
      <div className="table-container">
        <table>
          <thead><tr>{["Matricule","Nom complet","Classe","Sexe","Téléphone","Parent","Statut","Actions"].map(h => <th key={h}>{h}</th>)}</tr></thead>
          <tbody>
            {filtered.map(e => (
              <tr key={e.id}>
                <td><span className="badge badge-primary">{e.numero_matricule || "—"}</span></td>
                <td><strong>{e.prenom} {e.nom}</strong></td>
                <td>{getClasse(e.classe_id)}</td>
            ...
            {filtered.length === 0 && <tr><td colSpan={8} className="table-empty">Aucun élève trouvé</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
